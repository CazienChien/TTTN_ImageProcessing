...
def apply_morphology(operation):
    """ Áp dụng phép mở ảnh (Opening) hoặc đóng ảnh (Closing) bằng OpenCV """
    global img, edited_img
    if img:
        # Chuyển đổi ảnh PIL thành NumPy array
        img_array = np.array(img.convert("RGB"))  # Chuyển ảnh sang dạng RGB để xử lý

        # Chuyển đổi thành ảnh xám (grayscale) nếu cần
        gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)

        # Tạo kernel cho phép đóng/mở ảnh
        kernel = np.ones((5, 5), np.uint8)  # Kernel 5x5

        if operation == "OPEN":
            processed_img = cv2.morphologyEx(gray, cv2.MORPH_OPEN, kernel)  # Phép mở ảnh
        elif operation == "CLOSE":
            processed_img = cv2.morphologyEx(gray, cv2.MORPH_CLOSE, kernel)  # Phép đóng ảnh

        # Chuyển ảnh NumPy về dạng Pillow
        edited_img = Image.fromarray(processed_img)
...
filter_menu.add_command(label="Phép Mở Ảnh", command=lambda: apply_morphology("OPEN"))
filter_menu.add_command(label="Phép Đóng Ảnh", command=lambda: apply_morphology("CLOSE"))
